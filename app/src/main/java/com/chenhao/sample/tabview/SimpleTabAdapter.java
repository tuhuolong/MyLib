...
import app.lib.commonui.tabview.TabBaseAdapter;
...
public class SimpleTabAdapter extends TabBaseAdapter {
    ...
    protected void onSelectedTabView(View tabView, int position, boolean isSmooth) {
        ...
        if (isSmooth) {
            ...
            ObjectAnimator animTranslate = ObjectAnimator.ofFloat(cursor, "translationX",
                    cursor.getX(), tabView.getX() + offSize);
            ObjectAnimator animScale = ObjectAnimator.ofFloat(cursor, "scaleX", preScaleX,
    ...
    protected View onCreateCursor(View viewParent) {
        int cursorHeight = 2;
        RelativeLayout.LayoutParams paramsCursor = new RelativeLayout.LayoutParams(
                dip2px(viewParent.getContext(), cursorWidth),
