...
public class HorizontalTabView extends HorizontalScrollView {
    ...
    /**
     * A ScrollView is a {@link FrameLayout}, meaning you should place one child in it containing
     * the entire contents to scroll; holder is the only one child, and the parent for other views.
     */
    ...
    /**
     * if isSmoothShowEdgeItem == true HorizontalTabView can smoothly scroll the edge items to a
     * friendly vision position.
     */
    ...
    private void initTabView() {
        ...
        for (int i = 0; i < tabViewList.size(); i++) {
            if (i != 0) {
                ((RelativeLayout.LayoutParams) tabViewList.get(i).getLayoutParams()).addRule(
        ...
        tabViewList.get(0).getViewTreeObserver()
    ...
    /**
     * HorizontalTabView can smoothly scroll the edge items to a friendly vision position. {the
     * sizeOff position}
     *
     * @param sizeOff dp
     */
    ...
    private void setTabPosition(View view, int position, boolean isSmooth) {
        ...
        if (isSmoothShowEdgeItem && sizeOff > 0) {
            ...
            if (dx > sizeOff && dx < getWidth() - sizeOff - tabWidth)
                return;
            if (isSmooth) {
                smoothScrollBy(dx <= sizeOff ? (int) (dx - sizeOff)
                        : (int) (dx - getWidth() + sizeOff + tabWidth), 0);
            } else {
                scrollBy(dx <= sizeOff ? (int) (dx - sizeOff)
                        : (int) (dx - getWidth() + sizeOff + tabWidth), 0);
            }
        }
    }
    ...
    /**
     * if you want to change or add something (view) that will be included in the holder, you should
     * use this method to get the holder.
     *
     * @return the holder
     */
    ...
    /**
     * It has set a onClickListener for each View.But you can get the tab view, then you can use
     * setLongClickListener for every view or do anything you want to do;
     *
     * @return tabView
     */
